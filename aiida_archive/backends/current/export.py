...
def export(
    ...
    **kwargs: Any,
    ...
    try:
        if file_format == ExportFileFormat.TAR_GZIPPED:
            times = export_tar(
                entities=entities, filename=filename, silent=silent, **kwargs
            )
        else:  # zip
            times = export_zip(
                entities=entities,
...
def export_tar(
    ...
    **kwargs: Any,
    ...
    with SandboxFolder() as folder:
        ...
        with tarfile.open(
            filename, "w:gz", format=tarfile.PAX_FORMAT, dereference=True
        ) as tar:
...
def get_starting_node_ids(
    ...
    if GROUP_ENTITY_NAME in entities_starting_set:
        ...
        progress_bar.set_description_str(
            "Retrieving Nodes from Groups ...", refresh=True
        )
        ...
        qh_groups = (
            ...
            .append(
                orm.Group,
                filters={"uuid": {"in": entities_starting_set[GROUP_ENTITY_NAME]}},
                tag="groups",
            )
            .queryhelp
        )
        ...
        node_results = (
            orm.QueryBuilder(**qh_groups)
            .append(orm.Node, project=["id", "uuid"], with_group="groups")
            .all()
        )
        if node_results:
...
def get_entry_queries(
    ...
    if include_logs and node_ids_to_be_exported:
        ...
        builder.append(
            orm.Log,
            filters={"dbnode_id": {"in": node_ids_to_be_exported}},
            project="uuid",
        )
    ...
    for given_entity in given_entities:
        progress_bar.set_description_str(
            f"{pbar_base_str} - {given_entity}s", refresh=False
        )
        progress_bar.update()
        ...
        if not entry_uuids_to_add:
            ...
            entry_uuids_to_add.update(
                {node_pk_2_uuid_mapping[_] for _ in node_ids_to_be_exported}
            )

        builder = orm.QueryBuilder()
        builder.append(
            entity_names_to_entities[given_entity],
            filters={"uuid": {"in": entry_uuids_to_add}},
...
def get_export_data(
    ...
    for entity_name, partial_query in entries_queries.items():
        ...
        foreign_fields = {
            k: v for k, v in all_fields_info[entity_name].items() if "requires" in v
        }
...
def write_to_archive(
    ...
    nodesubfolder = folder.get_subfolder(
        NODES_EXPORT_SUBFOLDER, create=True, reset_limit=True
    )
    ...
    if all_node_uuids:
        ...
        for uuid in all_node_uuids:
            ...
            thisnodefolder = nodesubfolder.get_subfolder(
                sharded_uuid, create=False, reset_limit=True
            )

            # Make sure the node's repository folder was not deleted
            src = RepositoryFolder(
                section=Repository._section_name, uuid=uuid
